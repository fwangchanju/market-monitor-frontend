...
const BUTTON_CLASS =
  'flex h-7 w-7 items-center justify-center border-0 bg-transparent text-gray-400 outline-none hover:text-[var(--accent)]'
...
export default function NavBarPageActions({
  ...
  return (
    <>
      <button type="button" aria-label="새로고침" className={BUTTON_CLASS} onClick={() => window.location.reload()}>
        <RefreshIcon className="h-4 w-4" />
      </button>
      {showSnapshotControls && (
        <>
          <button type="button" aria-label="스냅샷 날짜" className={BUTTON_CLASS}>
            <CalendarIcon className="h-4 w-4" />
          </button>
          <button type="button" aria-label="스냅샷 시간" className={BUTTON_CLASS}>
