...
import { toJoEok } from '@/utils/format'
...
export default function MarketMapCategorySection({ category, path, onSelectCategory }: Props) {
  const { setNodeRef, isOver } = useDroppable({ id: category.categoryName })

  const ancestorPath = category.isSelf ? path.slice(0, -1) : path
  const label = `${[...ancestorPath, category.categoryName].join(' - ')} (시가총액: ${toJoEok(category.totalMarketValue / 100_000_000)})`
  ...
  return (
    ...
    >
      ...
      {category.boxes.map(box => (
        <MarketMapBox key={box.item.stockCode} item={box.item} x={box.x} y={box.y} width={box.width} height={box.height} />
      ))}
    </div>
  )
}
