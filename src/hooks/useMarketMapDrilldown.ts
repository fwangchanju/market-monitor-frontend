...
function findPathToCategory<T extends MarketMapCategoryNode>(
  nodes: T[],
  targetName: string,
  ancestors: string[] = [],
): string[] | null {
  for (const node of nodes) {
    ...
    const found = findPathToCategory(node.children as T[], targetName, path)
    if (found) return found
  }
  return null
}

export function useMarketMapDrilldown<T extends MarketMapCategoryNode>(rootNodes: T[]) {
