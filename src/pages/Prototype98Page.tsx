      ...
      <div style={{ maxWidth: '60%', margin: '0 auto' }}>
        ...
        <div className="window" style={{ marginBottom: 12 }}>
          <div className="title-bar">
            <div className="title-bar-text" style={{ fontSize: 16 }}>장 중 모니터링 — {data.snapshotTime?.slice(0, 16).replace('T', ' ')} | {data.marketStatus ?? 'N/A'}</div>
          </div>
        </div>
        ...
        <div className="window" style={{ gridColumn: '1 / -1' }}>
          <div className="title-bar"><div className="title-bar-text" style={{ fontSize: 16 }}>[0200]시장종합</div></div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'right', padding: '2px 6px' }}>상한</th>
                  <th style={{ textAlign: 'right', padding: '2px 6px' }}>상승</th>
                  <th style={{ textAlign: 'right', padding: '2px 6px' }}>보합</th>
                  <th style={{ textAlign: 'right', padding: '2px 6px' }}>하락</th>
                  <th style={{ textAlign: 'right', padding: '2px 6px' }}>하한</th>
                </tr>
              </thead>
              <tbody>
                {MARKETS.map(mkt => {
                  ...
                  return (
                    <tr key={item.marketType} style={{ borderTop: '1px solid #999' }}>
                      ...
                      <td style={{ textAlign: 'right', padding: '3px 6px' }}>{toIndex(item.indexValue)}</td>
                      ...
                      <td style={{ textAlign: 'right', padding: '3px 6px', color: '#555' }}>{toEokFromMln(item.tradingValue)}</td>
                      <td style={{ textAlign: 'right', padding: '3px 6px', color: RED }}>{toVolume(item.upperLimitCount)}</td>
                      <td style={{ textAlign: 'right', padding: '3px 6px', color: RED }}>{toVolume(item.advancers)}</td>
                      <td style={{ textAlign: 'right', padding: '3px 6px', color: '#555' }}>{toVolume(item.unchangedCount)}</td>
                      <td style={{ textAlign: 'right', padding: '3px 6px', color: BLUE }}>{toVolume(item.decliners)}</td>
                      <td style={{ textAlign: 'right', padding: '3px 6px', color: BLUE }}>{toVolume(item.lowerLimitCount)}</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        {/* 투자자별 매매종합 */}
        <div className="window" style={{ gridColumn: '1 / -1' }}>
          <div className="title-bar"><div className="title-bar-text" style={{ fontSize: 16 }}>[0780]투자자별매매동향-투자자별매매종합</div></div>
          <div className="window-body" style={{ padding: 8 }}>
            <p style={{ fontSize: 14, color: '#555', margin: '0 0 6px' }}>순매수 (단위: 억원)</p>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
              <thead>
                <tr>
                  <th style={{ textAlign: 'left', padding: '2px 6px' }}>시장</th>
                ...
                {MARKETS.map(mkt => (
                  <tr key={mkt} style={{ borderTop: '1px solid #999' }}>
                    ...
                    {INVESTORS.map(inv => {
                      ...
                      return (
                        <td key={inv} style={{ textAlign: 'right', padding: '3px 6px', ...pos(net) }}>
                          {toEokSigned(net)}
                        </td>
                      )
                    })}
        ...
        {/* 장중 투자자별 매매 */}
        <div className="window" style={{ gridColumn: '1 / -1' }}>
          <div className="title-bar"><div className="title-bar-text" style={{ fontSize: 16 }}>[1053]장중투자별매매상위</div></div>
          <div className="window-body" style={{ padding: 8 }}>
            <div style={{ display: 'flex', gap: 4, marginBottom: 6, flexWrap: 'wrap' }}>
              {EXCHANGES.map(m => <button key={m} style={btn(idMarket === m)} onClick={() => setIdMarket(m)}>{m}</button>)}
              <span style={{ margin: '0 4px', color: '#999' }}>|</span>
              {ID_INVESTORS.map(inv => <button key={inv} style={btn(idInvestor === inv)} onClick={() => setIdInvestor(inv)}>{investorLabel(inv)}</button>)}
              <span style={{ margin: '0 4px', color: '#999' }}>|</span>
              {(['NET_BUY', 'NET_SELL'] as IntradayRankingType[]).map(r => (
                <button key={r} style={btn(idRanking === r)} onClick={() => setIdRanking(r)}>{r === 'NET_BUY' ? '순매수' : '순매도'}</button>
              ))}
            </div>
            {idLoading ? (
              <p style={{ color: '#555', fontSize: 15 }}>불러오는 중...</p>
            ) : idItems.length === 0 ? (
              <p style={{ color: '#555', fontSize: 15 }}>데이터 없음</p>
            ) : (
                  ...
                  {idItems.map((item, idx) => {
                    ...
                    return (
                      <tr key={item.stockCode} style={{ borderTop: '1px solid #ccc' }}>
                        ...
                        <td style={{ padding: '2px 4px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.stockName}</td>
                        <td style={{ padding: '2px 4px', color: '#888' }}>{item.stockCode}</td>
                        <td style={{ textAlign: 'right', padding: '2px 4px', ...pos(netBuy) }}>{toMlnSigned(netBuy)}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div className="window">
          <div className="title-bar"><div className="title-bar-text" style={{ fontSize: 16 }}>[0766]프로그램매매-프로그램{rankP === 'NET_BUY' ? '순매수' : '순매도'}상위</div></div>
          <div className="window-body" style={{ padding: 8 }}>
            <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
              {(['NET_BUY', 'NET_SELL'] as ProgramRankingType[]).map(r => (
                <button key={r} style={btn(rankP === r)} onClick={() => setRankP(r)}>{r === 'NET_BUY' ? '순매수' : '순매도'}</button>
              ))}
            </div>
            {progLoading ? (
              <p style={{ color: '#555', fontSize: 15 }}>불러오는 중...</p>
            ) : progItems.length === 0 ? <p style={{ color: '#555', fontSize: 15 }}>데이터 없음</p> : (
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15, tableLayout: 'fixed' }}>
                <colgroup>
                  <col style={{ width: '6%' }} />
                  <col style={{ width: '24%' }} />
                ...
                <tbody>
                  {progItems.map((item, idx) => (
                    <tr key={item.stockCode} style={{ borderTop: '1px solid #ccc' }}>
                      ...
                      <td style={{ padding: '2px 4px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.stockName}</td>
                      <td style={{ padding: '2px 4px', color: '#888' }}>{item.stockCode}</td>
                      <td style={{ textAlign: 'right', padding: '2px 4px', ...pos(item.programNetBuyAmount) }}>{toMlnSigned(item.programNetBuyAmount)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 4px', color: '#555' }}>{toVolume(item.programBuyAmount)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 4px', color: '#555' }}>{toVolume(item.programSellAmount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* 지수 기여도 상위 */}
        <div className="window">
          <div className="title-bar"><div className="title-bar-text" style={{ fontSize: 16 }}>[1136]지수기여도상위</div></div>
          <div className="window-body" style={{ padding: 8 }}>
            <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
              {MARKETS.map(m => <button key={m} style={btn(mktC === m)} onClick={() => setMktC(m)}>{m}</button>)}
            </div>
            {contribLoading ? (
              <p style={{ color: '#555', fontSize: 15 }}>불러오는 중...</p>
            ) : contribItems.length === 0 ? <p style={{ color: '#555', fontSize: 15 }}>데이터 없음</p> : (
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15, tableLayout: 'fixed' }}>
                <colgroup>
                  <col style={{ width: '7%' }} />
                  <col style={{ width: '33%' }} />
                ...
                <tbody>
                  {contribItems.map(item => (
                    <tr key={item.rank} style={{ borderTop: '1px solid #ccc' }}>
                      ...
                      <td style={{ padding: '2px 6px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.stockName}</td>
                      <td style={{ padding: '2px 6px', color: '#888' }}>{item.stockCode}</td>
        ...
        <div className="window" style={{ gridColumn: '1 / -1' }}>
          <div className="title-bar">
            <div className="title-bar-text" style={{ fontSize: 16 }}>[0142]종목별공매도추이{shortCode ? ` — ${stockLabel(shortCode)}` : ''}</div>
          </div>
          <div className="window-body" style={{ padding: 8 }}>
            {!shortCode ? (
              <p style={{ color: '#555', fontSize: 15 }}>관심종목 없음</p>
            ) : shortLoading ? (
              <p style={{ color: '#555', fontSize: 15 }}>불러오는 중...</p>
            ) : shortItems.length === 0 ? (
              <p style={{ color: '#555', fontSize: 15 }}>데이터 없음</p>
                ...
                <tbody>
                  {shortItems.map(item => (
                    <tr key={item.tradeDate} style={{ borderTop: '1px solid #ccc' }}>
                      <td style={{ padding: '2px 6px', color: '#555' }}>{toYyMmDd(item.tradeDate)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px' }}>{toVolume(item.closePrice)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', ...pos(item.priceChange) }}>{toPctSigned(item.changeRate)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', color: '#555' }}>{toVolume(item.shortVolume)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', color: '#555' }}>{toPct(item.shortRatio)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', color: '#555' }}>{toVolume(item.shortAmount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="window" style={{ gridColumn: '1 / -1' }}>
          <div className="title-bar">
            <div className="title-bar-text" style={{ fontSize: 16 }}>[0778]프로그램매매추이-종목별{shortCode ? ` — ${stockLabel(shortCode)}` : ''}</div>
          </div>
          <div className="window-body" style={{ padding: 8 }}>
            {!shortCode ? (
              <p style={{ color: '#555', fontSize: 15 }}>관심종목 없음</p>
                ...
                <tbody>
                  {progHistItems.map(item => (
                    <tr key={item.snapshotTime} style={{ borderTop: '1px solid #ccc' }}>
                      <td style={{ padding: '2px 6px', color: '#555' }}>{toTimeLabel(item.snapshotTime)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', ...pos(item.programNetBuyAmount) }}>{toMlnSigned(item.programNetBuyAmount)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', color: '#555' }}>{toVolume(item.programBuyAmount)}</td>
                      <td style={{ textAlign: 'right', padding: '2px 6px', color: '#555' }}>{toVolume(item.programSellAmount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
