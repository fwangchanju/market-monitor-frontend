...
### 결정 5 — 렌더러 타임아웃은 백엔드 레포에서 따로 올린다

`waitForSelector` 15초를 20초로 늘리기로 했다(사용자 결정). **다만 이 작업에 포함되지 않는다** —
렌더러는 백엔드 레포(`containers/renderer/server.js`)이고 배포 대상도 `target=renderer`로 다르다.
그쪽 지시서가 따로 있다.

**여기서 고치는 것이 원인이고, 타임아웃은 완충이다.** 둘은 독립이라 어느 쪽이 먼저 나가도 된다.

실측 근거를 남겨둔다(2026-09-22 장중, 렌더러 서버에서 측정).

```
/api/sector?market=ALL_STOCK   7.11  7.26  8.29 초   ← 버려지는 조회
/api/sector?market=KOSPI       3.27  2.90  2.67 초   ← 실제로 쓰는 것
/api/map?market=ALL_STOCK      2.89  2.29  2.99 초
/api/map?market=KOSPI          2.32  2.05  2.48 초
```

**아무도 안 보는 조회가 15초 예산의 절반을 먹고 있다.** 이 작업이 되돌려받는 것이 그 절반이다.
...
- **렌더러·백엔드를 건드리지 않는다** (결정 5). 타임아웃 조정은 백엔드 레포의 별도 지시서다.
  이 작업은 프론트 레포 하나로 끝난다
...
**0건이 나와도 고쳐졌다는 증거는 아니다.** 9/21은 26틱 중 11건이 죽었는데 9/22는 같은 코드로 오전
6틱이 전부 통과했다. 배포된 것이 없는데도 그렇다. 즉 **이 실패는 15초 경계 바로 옆에서 나는
간헐적인 것**이지 매번 나는 것이 아니다.

그래서 판정은 하루가 아니라 **여러 날**로 본다. 되돌려받는 것은 "7.6초짜리 조회 한 건"이고, 그만큼
경계에서 멀어지는 것이 이 작업의 성과다. 며칠 지켜봐도 9/21 같은 날이 다시 오면 조회 개수 말고
다른 원인이 있다는 뜻이라 그때 다시 본다.
